...
def vision_catch_cup(fr5_B):
    ...
    if vision_arm_test.cup_position == [[],[],[]]:
        ...
        print('enter this')
        vision_servo2(vision_arm_test.cup_loc_to_base)
        time.sleep(0.001)
        ...
        if all(abs(delta) < change_threshold for delta in delta_xyz):# 如果xyz坐标的变化量小于阈值，认为目标物体稳定
            ...
            while(y_flag):
                if (fr5_B_end[1] - b_bias - current_xyz[1]) > 5.0:
                    ...
                    fr5_camera2baseB_end_from_SDK = fr5_B.robot.GetActualToolFlangePose(0)
                    while( type(fr5_B_end_from_SDK) != tuple):
                        fr5_B_end_from_SDK = fr5_B.robot.GetActualToolFlangePose(0)
                        time.sleep(0.008)
                    print("fr5_B_end from SDK",fr5_B_end_from_SDK)# 得到机械臂末端xyz，rxryrz
...
def vision_servo2(catch_xyz):
    ...
    if(current_xyz[0] != 0 and current_xyz[1] != 0 and current_xyz[2] != 0 ):
        ...
        if(fr5_B_end[0] - current_xyz[0] > 20.0 or (fr5_B_end[0] - current_xyz[0] < -20.0)):
            ...
            # x_from_start2end = fr5_B_end[0] - current_xyz[0]# 可能是正值或负值，但在后面会被兼容
            # y_from_start2end = fr5_B_end[1] - current_xyz[1] - b_bias# 从二指中心到目标物体的y方向偏移量，可能是正值或负值，但在后面会被兼容
            ...
            # print('x_from_start2end.{},y_from_start2end.{}',format(x_from_start2end),format(y_from_start2end))
            ...
            for i in range(int(len(trajectory)*0.5)):
                ...
                fr5_B_jointpos_new = fr5_B_jointpos_new[1]
                # 执行servoJ
                fr5_B.robot.ServoJ(fr5_B_jointpos_new,0.0,0.0,0.016,0.0,0.0)
                time.sleep(0.016)
            # exit()
        
        # 如果x没对齐不是很严重，这个时候只需要在x轴上移动就行了
        if( ((fr5_B_end[0] - current_xyz[0] < 20.0) and (fr5_B_end[0] - current_xyz[0] > 3.0)) ):
            print('执行x负方向不断跟进')
            print("current_xyz[0]:",current_xyz[0])
            ...
            current_xyz[2] = float(vision_arm_test.cup_loc_to_base[2])
            time.sleep(0.016)
        elif ((fr5_B_end[0] - current_xyz[0] < -3.0) and (fr5_B_end[0] - current_xyz[0] > -20.0)):
            print('执行x负方向不断跟进')
            print("current_xyz[0]:",current_xyz[0])
        ...
        if( (fr5_B_end[0] - current_xyz[0] < 5.0 and (fr5_B_end[0] - current_xyz[0] > -5.0)) and ((fr5_B_end[1] - current_xyz[1] - b_bias > 10.0)or (fr5_B_end[1] - current_xyz[1] - b_bias < -10.0) ) ):
            print('执行y负方向不断跟进')
            current_xyz[0] = float(vision_arm_test.cup_loc_to_base[0])
            current_xyz[1] = float(vision_arm_test.cup_loc_to_base[1])
            current_xyz[2] = float(vision_arm_test.cup_loc_to_base[2])
        ...
        current_xyz[1] = float(vision_arm_test.cup_loc_to_base[1])
        current_xyz[2] = float(vision_arm_test.cup_loc_to_base[2])
        if ( ((fr5_B_end[0] - current_xyz[0] < 5.0) and (fr5_B_end[0] - current_xyz[0] > -5.0)) and (fr5_B_end[1] - current_xyz[1] - b_bias < 10.0)and (fr5_B_end[1] - current_xyz[1] - b_bias > -10.0)      ):
            print("x,y对齐完成")
            fr5_B.robot.MoveGripper(1, 0, 50, 10, 10000, 1) # 夹爪闭合
            time.sleep(3)
            fr5_B.MoveL(0.0,0.0,50.0,40.0)
...
if __name__ == "__main__":
    ...
    init()
    # 加载模型
    model = joblib.load('/home/zjh/FR5_exp/scripts/fr5init/model.pkl')
    
    vision_arm_test.talker()
    # testik()
    time.sleep(1)

    fr5_B.robot.MoveCart([vision_arm_test.cup_loc_to_base[0],vision_arm_test.cup_loc_to_base[1] + b_bias + 100.0,63.0,90.0,0.0,0.0], 0, 0)
